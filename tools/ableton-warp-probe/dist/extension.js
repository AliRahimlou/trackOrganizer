...
function warpModeName(mode) {
  return WarpMode[mode] ?? String(mode);
}
function stableName(audioPath) {
  return `${crypto.createHash("sha1").update(audioPath).digest("hex").slice(0, 20)}.json`;
}
async function writeIfPossible(filePath, text) {
  try {
...
async function writePayload(context, payload) {
...
function payloadForClip(context, clip) {
  return {
...
async function exportSong(context) {
  let exported = 0;
...
function activate(activation) {
  ...
  context.commands.registerCommand("trackOrganizer.exportClipWarpMarkers", (arg) => {
    void (async () => {
      const clip = context.getObjectFromHandle(arg, Clip);
      if (!(clip instanceof AudioClip)) {
  ...
  context.commands.registerCommand("trackOrganizer.exportSelectionWarpMarkers", (arg) => {
    void (async (selection) => {
      const tracks = selection.selected_lanes.map((handle) => context.getObjectFromHandle(handle, DataModelObject)).filter((obj) => obj instanceof AudioTrack);
      let exported = 0;
    ...
    })(arg).catch((error) => console.error(error));
  });
  context.commands.registerCommand("trackOrganizer.exportTrackWarpMarkers", (arg) => {
    void (async () => {
      const track = context.getObjectFromHandle(arg, DataModelObject);
      if (!(track instanceof AudioTrack)) {
  ...
  context.commands.registerCommand("trackOrganizer.exportSlotWarpMarkers", (arg) => {
    void (async () => {
      const slot = context.getObjectFromHandle(arg, ClipSlot);
      const clip = slot.clip;
      if (!(clip instanceof AudioClip)) {
  ...
  context.commands.registerCommand("trackOrganizer.exportSelectedSlotWarpMarkers", (arg) => {
    void (async (selection) => {
      let exported = 0;
