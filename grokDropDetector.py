...
# =================== DROP DETECTOR ===================
def detect_drops_beats(drums_path: str, bpm: int) -> List[Tuple[float, float]]:
    """
    Detect multiple energy transitions ("drops").
    """
    if not _HAS_LIBROSA:
    ...
    try:
        ...
        spb = 60.0 / max(1, bpm)
        t_frames = np.arange(len(rms)) * (hop / sr)
        ...
        full_db  = 10*np.log10(np.maximum(full_lin_beats, 1e-12))
        ...
        if SMOOTH_BEATS > 0:
            ...
            full_db = _moving_mean(full_db, k)

        median_share = np.median(share)
        bass_weight = min(1.0, max(0.3, median_share / 0.1))
        combined_db = bass_weight * low_db + (1 - bass_weight) * full_db
        ...
        bar_combined = R4(combined_db).mean(axis=1).astype(float)
        ...
        p10 = float(np.percentile(bar_combined[seg], 10))
        p90 = float(np.percentile(bar_combined[seg], 90))
        spread = max(6.0, p90 - p10)          # stabilize
        quiet_thr = float(np.percentile(bar_combined[seg], 35))  # what counts as "valley"
        ...
        if DEBUG_DROPS:
            print(f"   [auto] window bars {start_bar}-{end_bar-1} | quiet≈{quiet_thr:.2f} | spread≈{spread:.2f} | step≥{step_thr:.2f} | bass_weight={bass_weight:.2f}")
        ...
        for s in range(start_bar + BASELINE_BARS, end_bar - SUSTAIN_BARS):
            ...
            pre_med  = float(np.median(bar_combined[pre_slice]))
            pre_min  = float(np.min(bar_combined[max(start_bar, s-VALLEY_LOOKBACK_BARS):s]))
            post_mean = float(np.mean(bar_combined[post_slice]))
            ...
            # keep best as potential fallback
            if valley_ok and score > best_score:
                best_score, best_bar = score, s

        if not candidates and best_bar is not None:
            candidates = [(best_bar, best_score)]

        # filter for separation, prefer higher score if close
        if candidates:
            candidates.sort()  # by bar
            filtered = []
            for s, score in candidates:
                if not filtered or s - filtered[-1][0] >= MIN_DROP_SEP_BARS:
                    filtered.append((s, score))
                elif score > filtered[-1][1]:
                    filtered[-1] = (s, score)
            filtered = filtered[:MAX_DROPS]
        else:
            return []

        # ---- beat-level refine for each ----
        drops = []
        for chosen_bar, _ in filtered:
            ...
            drops.append((drop_sec, drop_beats))
...
def modify_als_file(input_path: Optional[str], target_folder: str, track_names: Dict[str, Optional[str]], bpm_value: Optional[int], force: bool=False) -> None:
    ...
    if bpm_value and drums_abs and os.path.exists(drums_abs):
        ...
        drops = detect_drops_beats(drums_abs, bpm_value)
    else:
        if not bpm_value:
    ...
    if drops:
        first_drop_sec, first_drop_beats = drops[0]
        _set_clip_timing_fields(root, first_drop_beats, loop_end_beats=loop_end_beats)
        for i, (drop_sec, drop_beats) in enumerate(drops, 1):
            _add_or_update_warp_marker_for_all_clips(root, drop_sec=drop_sec, drop_beats=drop_beats)
            # Add locator
            locators_parent = root.find(".//Locators")
            if locators_parent is None:
                arranger = root.find(".//Arranger")
                if arranger is None:
                    arranger = ET.SubElement(root, "Arranger")
                locators_parent = ET.SubElement(arranger, "Locators")
            locator = ET.SubElement(locators_parent, "Locator")
            time_elem = ET.SubElement(locator, "Time")
            time_elem.set("Value", f"{drop_beats:.6f}")
            name_elem = ET.SubElement(locator, "Name")
            name_elem.set("Value", f"Drop {i}")
            print(f"   ➕ Drop {i} stamped to ALL clips at ~bar {drop_beats/4:.2f}.")
    else:
