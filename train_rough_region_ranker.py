...
"""
Train a lightweight logistic rough-region classifier from custom candidate rows.

The resulting JSON model can be consumed by ``first_downbeat_detector.py`` via:

- ``DetectorOptions(rough_region_model_path="...")``
- or ``DOWNBEAT_ROUGH_REGION_MODEL=/path/to/model.json``
"""
...
from first_downbeat_detector import ROUGH_REGION_MODEL_FEATURES


LOG = logging.getLogger("train_rough_region_ranker")
...
def _prepare_matrix(rows: Sequence[Dict[str, object]], label_col: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for row in rows:
        ...
        feats: List[float] = []
        valid = True
        for name in ROUGH_REGION_MODEL_FEATURES:
    ...
    if not x_rows:
        return np.zeros((0, len(ROUGH_REGION_MODEL_FEATURES)), dtype=np.float64), np.zeros((0,), dtype=np.float64)
...
def train_model(
    *,
    ...
    rows = _read_rows(rough_candidate_csv)
    train_rows, valid_rows = _group_split(rows, valid_frac=valid_frac)
    x_train, y_train = _prepare_matrix(train_rows, label_col)
    x_valid, y_valid = _prepare_matrix(valid_rows, label_col)
    ...
    payload: Dict[str, object] = {
        ...
        "feature_order": list(ROUGH_REGION_MODEL_FEATURES),
        ...
        "source_rough_candidate_csv": os.path.abspath(rough_candidate_csv),
    }
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Train a simple logistic rough-region classifier from custom candidate CSV rows.")
    ap.add_argument("--rough-candidate-csv", required=True, help="CSV produced by evaluate_downbeat_detector.py --out-rough-candidate-csv")
    ap.add_argument("--out-model", required=True, help="Output JSON model path")
    ap.add_argument("--label-col", default="candidate_is_manual_match_1beat", help="Binary label column to learn")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    payload = train_model(
        rough_candidate_csv=args.rough_candidate_csv,
